$(document).ready(function () {
    ...
    $('.offices-block .city-name').click(function () {
        $('.window-city-wrap').fadeIn();
    });
    $('.window-city-wrap').click(function (event) {
        $target = $(event.target);
        if (!$target.closest($('.window-city')).length) $('.window-city-wrap').fadeOut();
        if ($target.hasClass('close-window')) $('.window-city-wrap').fadeOut();
    });


    $('.offices-list').niceScroll({
        cursorborderradius: "0px",
        cursorborder: "1px solid #e9e9e9",
        cursoropacitymin: 1,
        cursoropacitymax: 1,
        cursorcolor: "#f1f1f1",
        cursorwidth: "8px",
    ...
    $('.city-menu .sub-menu li span').click(function(){
        if(!$(this).hasClass('active')){
            $('.city-menu .sub-menu li span').removeClass('active');
            $(this).addClass('active');
